...
### Important: Cross-File Dependencies When Merging (does this section belong in AGENTS.md?)

When merging Mozilla updates, be aware of these critical cross-file dependencies:

#### HTML ↔ JavaScript Configuration
- `web/viewer.html` button IDs must match `web/viewer.js` configuration
- Example: If viewer.html has `id="presentationMode"`, viewer.js must have `getElementById("presentationMode")`
- **Files to check together**: `web/viewer.html` + `web/viewer.js`

#### CSS ↔ HTML Classes
- Custom CSS classes (mcToolbar, mcViewer, mcSidebar, mcThumbnail) must be applied in HTML
- **Files to check together**: `web/viewer.css` + `web/viewer.html` + `web/pdf_thumbnail_view.js`

#### Business Logic ↔ UI
- MCOptions usage must match UI elements
- Print permission checks must align with print button visibility
- **Files to check together**: `web/app.js` + `web/viewer.html`

#### After Merge Validation Checklist
1. Run `gulp generic` to ensure it builds without errors
2. Open the viewer in a browser and check browser console for errors
3. Verify all toolbar buttons load without null reference errors
4. Test that hidden buttons (print, download, annotations) remain hidden
5. Test that custom classes (mcToolbar, etc.) are applied and styled correctly
